...
myPlotly <- function(dt, title, caption = '@williamfang') {
  temp <- gather(dt[,.(TradingDay, BrokerID, ClassID, total, totalPct)], 
...
myPlotly(dtVol, title = 'Volume for top10Pos')
myPlotly(dtPos, title = 'Position for top10Vol')
...
myPlotly(dtAll, title = 'Position for top10Vol <==> Volume for top10Pos')
